...
As shown in plot 3, total means of estimation of different beta1 are approximately equal to true parameter, since we simulated 10,000 times with fairly large sample size, but the means of estimation which are rejected of different beta1 are gradually approching to true parameter as the effective size increases.

When effective size is close to 0, it is hard to reject the null hypothesis unless the estimation value falls in extremely situation (tails of true distribution), and estimation in extremly large situation is more likely to be generated and rejected, thus the proportion of rejection is small (as shown in plot 2) and the mean value is larger than the true parameter.
